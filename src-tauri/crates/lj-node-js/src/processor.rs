...
impl NodeProcessor for JsNodeProcessor {
    ...
    fn output_type(&self) -> NodeDataVariant {
        NodeDataVariant::Json // JS 结果优先作为结构化中间数据输出
    }
            ...
            match rx.recv().await {
                Some(Ok(json_str)) => {
                    match serde_json::from_str::<serde_json::Value>(&json_str) {
                        Ok(value) => yield NodeData::Json(value),
                        Err(_) => yield NodeData::Raw(json_str),
                    }
                }
                Some(Err(e)) => yield NodeData::Error(format!("JS 错误: {e}")),
