...
pub fn extract_single(
    ...
    match try_extract_on_doc(&doc, rules, regex_cache) {
        Ok(text) if !text.is_empty() => vec![NodeData::Json(serde_json::json!({ "title": text }))],
        Ok(_) | Err(_) => {
...
pub fn extract_toc_list(
    ...
    let doc = html::parse_html(body_str);
    let Ok(items) = collect_list_elements(&doc, rules, "媒体单元列表模式") else {
        return vec![];
    };
