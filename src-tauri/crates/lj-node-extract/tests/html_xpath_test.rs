...
fn json_at(result: &[NodeData], index: usize) -> &serde_json::Value {
    match &result[index] {
        NodeData::Json(value) => value,
        other => panic!("期望 Json 中间记录, 得到 {other:?}"),
    }
}
...
fn test_html_xpath_single_title() {
    ...
    assert_eq!(json_at(&result, 0)["title"], "测试标题");
}
...
fn test_html_xpath_list() {
    ...
    assert_eq!(json_at(&result, 0)["title"], "书1");
    assert_eq!(json_at(&result, 0)["author"], "作者1");
    assert_eq!(json_at(&result, 1)["title"], "书2");
    assert_eq!(json_at(&result, 1)["author"], "作者2");
}
...
fn test_html_xpath_empty_body() {
    ...
    let result = html_xpath::extract_single(html, &rules, &cache);
    assert_eq!(result.len(), 1);
    assert!(matches!(&result[0], NodeData::Error(_)));
}
...
fn test_html_xpath_regex_clean() {
    ...
    let result = html_xpath::extract_single(html, &rules, &cache);
    assert_eq!(json_at(&result, 0)["title"], "价格: 100元");
}

/// 章节列表模式：用 `XPath` 提取章节中间记录。
#[test]
fn test_html_xpath_unit_list() {
    let html = r"
        <div id='chapters'>
    ...
    assert_eq!(result[0]["url"], "http://example.com/chapter/1.html");
