...
use lj_core::endpoint::{EndpointKind, HttpMethod};
...
impl NodeProcessor for HttpNodeProcessor {
    ...
    fn process<'a>(
        ...
        Box::pin(stream! {
            ...
            let key = match first_item {
                Some(NodeData::Raw(k)) => Some(k),
                _ => None,
            };

            // 按端点类型决定模板变量(KTD1:Detail 段传 vod_id 支持视频源)
            let (tpl_key, tpl_book_url, tpl_chapter_url, tpl_vod_id) =
                match http_spec.endpoint_kind {
                    EndpointKind::Search | EndpointKind::Discover => {
                        (key.as_deref(), None, None, None)
                    }
                    EndpointKind::Detail | EndpointKind::Toc => {
                        // 视频 Detail 优先 vod_id(URL 模板含 {{vod_id}} 则替换),
                        // 书源 Detail 用 bookUrl(URL 模板含 {{bookUrl}} 则替换)。
                        // 两者互斥:同一 Graph 不会同时含两种模板。
                        (None, key.as_deref(), None, key.as_deref())
                    }
                    EndpointKind::Content => (None, None, key.as_deref(), None),
                };
            ...
            let url_str = util::render_url_template(
                ...
                tpl_chapter_url,
                tpl_vod_id,
                None, // {{type}}: Maccms Discover 分类,首刀前端不填替换为空走全站最新流(R10)
            );
