...
#[expect(clippy::needless_pass_by_value, reason = "Tauri 命令参数必须 owned")]
#[tauri::command]
pub fn import_rule_with_preview(
    request: ImportPreviewRequest,
) -> Result<ImportPreviewResponse, String> {
    let preview = detect_and_import(&request.rule_json, request.maccms_format)?;
...
pub fn confirm_import(
    ...
    let graph: Graph =
        serde_json::from_str(&request.graph_json).map_err(|e| format!("反序列化图失败: {e}"))?;
...
#[expect(clippy::needless_pass_by_value, reason = "Tauri 命令参数必须 owned")]
#[tauri::command]
pub fn list_rules(state: State<AppState>) -> Result<Vec<RuleListItem>, String> {
    let storage = state
        .storage
        .lock()
        .map_err(|e| format!("存储锁获取失败: {e}"))?;
...
pub async fn execute_segment(
    ...
    let endpoint_kind = request.endpoint_kind;
...
mod tests {
    ...
    fn dispatch_native_graph_json() {
        // 空图(无端点子图)通过 schema 校验:无 endpoint_group 则跳过模板检查。
